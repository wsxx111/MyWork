...
/*
 * 将一个系统分割成一系列相互协作的类有过一哥很不好的副作用，那就是需要维护相关对象间的一致性，我们不希望为了维持一致性而使各个类紧密耦合，这样会给维护，扩展和重用都带来不便。而观察者模式的关键对象是主题Subject和观察者Obverser，一个Subject可以任意数目依赖它的Obverser,一旦Subject的状态发生变化，所有的Obverser都合一得到通知。Subject发出通知时，并不需要知道谁是它的观察者，也就是说，具体观察者是谁，它根本不需要知道，而任何一个具体的观察者不知道也不需要知道其他观察者的存在。
 * 
